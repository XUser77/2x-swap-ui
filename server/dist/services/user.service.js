...
export class UserStatsService {
    static async getSeasonStatus(userId) {
        ...
        const percentile = userIndex === -1 ? null : ((userIndex + 1) / totalRankedUsers) * 100;
        /** Rank badge */
        let rankName = "Unranked";
        let nextRankAtPercentile = null;
        if (percentile !== null) {
        ...
        /** Days remaining */
        const endsInDays = Math.max(0, Math.ceil((season.endAt.getTime() - Date.now()) / (1000 * 60 * 60 * 24)));
        return {
            season: {
                id: season.id,
                name: season.name,
