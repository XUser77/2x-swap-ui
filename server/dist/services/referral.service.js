...
class ReferralService {
    ...
    static async getReferralStats(userId) {
        ...
        const activeCount = activeInvitees.filter((u) => u.trades.length > 0 || u.liquiditySnapshots.length > 0).length;
        return {
    ...
    static async getReferralActivity(userId) {
        ...
        return invitees.map((invitee) => {
            const tradeVolume = invitee.trades.reduce((sum, t) => sum + Number(t.volume), 0);
            const lpVolume = invitee.liquiditySnapshots.reduce((sum, l) => sum + Number(l.avgDailyBalance), 0);
            let activityType = "None";
            if (tradeVolume > 0 && lpVolume > 0)
                activityType = "Mixed";
